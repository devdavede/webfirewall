...
class Firewall {
    private $database;
    private $blocklist = "blocked_ips.txt";
    private $blockAfterAttempts = 3;
    ...
    function blockIP($ip) {
        $blacklist_content = file_get_contents($this->blocklist);
    ...
    function logInvalidAccess() {
        ...
        if(count($entries) > $this->blockAfterAttempts) {
            $this->blockIP($ip);
        }
